...
//Recolectamos los comentarios del articulo
...
<body style="background-image: url('img/fondoG.jpeg'); background-size:cover; background-attachment:fixed">
    <div class="container">
        ...
        <div class="card" style="background:linear-gradient(rgba(46, 17, 83,0.5),rgba(12, 67, 131,0.5))">
            <div class="card-body" style="background:linear-gradient(rgba(46, 17, 83,0.3),rgba(12, 67, 131,0.3)); border-radius: 15px">
                <br>
            ...
            <div class="card-body" style="background:linear-gradient(rgba(46, 17, 83,0.3),rgba(12, 67, 131,0.3)); border-radius: 15px">
                ...
                <div class="row" style="align:center; padding:0px">
                    <div class="offset-md-2 col-md-8 offset-md-2">
                        <h3 style="color: white; text-align: center">Comentarios</h3>
                        <?php
                        if ($resultadoC) {
                            while ($comentarios = mysqli_fetch_array($resultadoC)) {
                                //Datos del comentario
                                $correoC = $comentarios["correo"];                                      
                                $estatus_comentario = $comentarios["estatus_comentario"];
                                $contenidoC = $comentarios["contenido"];
                                
                                if ($estatus_comentario == 'Publicado') {
                                    //Datos del lector del comentario
                                    $query = "SELECT * FROM usuarios WHERE correo = '$correoC'";
                                    $res = mysqli_query($conn, $query);
                                    if ($res->num_rows == 1) {
                                        $lector = mysqli_fetch_assoc($res);
                                    }
